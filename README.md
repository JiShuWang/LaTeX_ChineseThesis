# LaTeX_ChineseThesis
一个用于国内研究生学位论文撰写的LaTeX模板及其基本使用教程。该模板最初是用于云南大学研究生学位论文的撰写，但国内的学位论文在很多格式上（如字体）基本一致，因此也可根据自己的需求扩展到各个高校的研究生学位论文撰写。如有任何问题和建议，欢迎Issue。

使用LaTeX编写学位论文，不仅可以符合长期使用LaTeX编写学术论文的研究者的习惯，也可以同时避免使用Word撰写论文过程中经常面临的图表位置错乱、模板不一致、参考文献格式不统一等问题，从而提高论文撰写效率，并实现标准的论文格式。

该模板将不定期更新，更新方式包括可选和默认。可选更新表示增加了一些可选功能，其代码不会直接添加到源文件中，可参考Optional.tex来选择需要的功能并复制代码到自己的项目中。而默认更新则表示添加了一些通用功能（如缩写表），此类更新会直接改动源文件。

## 2025.12.30 更新（可选）
改动文件：main.tex

改动内容：添加json格式的代码显示，详情见Optional.tex
